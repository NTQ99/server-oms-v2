...
import static ntq.server.common.util.JsonUtil.*;
...
public class JsonObject implements Iterable<Map.Entry<String, Object>> {
  ...
  public String getString(String key) {
    ...
    } else if (val instanceof byte[] buf) {
      return BASE64_ENCODER.encodeToString(buf);
    } else if (val instanceof Enum em) {
      return em.name();
    } else {
      return val.toString();
    }
  ...
  public Double getDouble(String key) {
    ...
    if (number == null) {
      ...
      return d;  // Avoids unnecessary unbox/box
    } else {
  ...
  public Float getFloat(String key) {
    ...
    if (number == null) {
      ...
      return f;  // Avoids unnecessary unbox/box
    } else {
  ...
  private void fromJson(String json) {
    map = Json.decode(json.getBytes(StandardCharsets.UTF_8), Map.class);
  }
